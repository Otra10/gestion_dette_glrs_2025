...
public class DemandeDetteRepositoryDb extends RepositoryDb<DemandeDette> implements IDemandeDetteRepository {
    ...
    public List<DemandeDette> findDemandesByClientId(int clientId) {
        ...
        String sql = "SELECT * FROM demande_dette WHERE client_id = ?"; // La requête SQL

        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, clientId); // Définit le paramètre pour le clientId

            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    // Récupération des informations de la demande de dette depuis le ResultSet
                    int id = resultSet.getInt("id");
                    LocalDate date = resultSet.getDate("date").toLocalDate(); // Convertir en LocalDate
                    double montantTotal = resultSet.getDouble("montantTotal");
                    EtatDemandeDette etat = EtatDemandeDette.valueOf(resultSet.getString("etat")); // Convertir en enum

                    // Créer un objet DemandeDette avec les données récupérées
        ...
        return demandes; // Retourne la liste des demandes
    }
    //
}
